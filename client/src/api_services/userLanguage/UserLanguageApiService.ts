// src/api_services/userLanguage/UserLanguageApiService.ts

import axios from "axios";
import type { ApiResponseUserLanguageLevel, ApiResponseLanguagesList } from "../../types/userLanguage/ApiResponseUserLanguage";

const API_URL_ADD: string = import.meta.env.VITE_API_URL + "userLanguagesAdd";
const API_URL_MISSING: string = import.meta.env.VITE_API_URL + "userLanguagesMissing";
...
export const userLanguageLevelApi = {
  async dodajUserLanguageLevel(userId: number, jezik: string, nivo: string): Promise<ApiResponseUserLanguageLevel> {
    try {
      ...
      const response = await axios.post<ApiResponseUserLanguageLevel>(
        API_URL_ADD,
        { userId, jezik, nivo },
        config
      );
  ...
  async getLanguagesUserDoesNotHave(userId: number): Promise<ApiResponseLanguagesList> {
    try {
      ...
      const response = await axios.get<ApiResponseLanguagesList>(
        API_URL_MISSING,
        config
      );
